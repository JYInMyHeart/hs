...
do_readline ([MalString prompt]) = do
  str <- lift $ getInputLine prompt
  case str of
    Nothing -> return $ MalString ""
    Just s  -> return $ MalString s
...
dissoc (MalHashMap hm _ : ks) =
  let remover = (\hm (MalString k) -> Map.delete k hm)
  in  return $ MalHashMap (foldl remover hm ks) Nil
...
contains_Q (MalHashMap hm _ : MalString k : []) = do
  if Map.member k hm then return MalTrue else return MalFalse
...
with_meta ((MalFunc { fn = f, ast = a, env = e, params = p, macro = mc }) : m : [])
  = do
    return
      $ MalFunc {fn = f, ast = a, env = e, params = p, macro = mc, meta = m}
